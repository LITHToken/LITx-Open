...
describe("FeeDsitributor", function () {
  ...
  const merkleRoot = "0xd1c71cd4cee6b17d824c48a06d234ec60a4f2400e5dd6f13ed22b4c714859f27";
  const me = "0x8e8A4724D4303aB675d592dF88c91269b426C62a";
  const pk0 = "df57089febbacf7ba0bc227dafbffa9fc08a93fdc68e1e42411a14efcf23656e";
  const pk1 = "de9be858da4a475276426320d5e9262ecfc3ba460bfac56360bfa6c4c28b4ee0";
  const claim0 = 0x0100;
  const claim1 = 0x0101;
  const proof0 = ["0x9d372dd6998665eae9f3b57de4fc67bf3572da22ee0dc52b2af12e20b777c9ba"];
  const proof1 = ["0x56ae380ed1131fbaff773d57c6da798e5c07dc4588582f3f5204120cf59bc6d1"];
  const index0 = 1;
  const index1 = 0;

  beforeEach(async function () {
    [deployer, bridge, ecosystem, marketing] =
    ...
    rcpt = await (await feedistributor.setMerkleRoot(merkleRoot)).wait();
    expect(rcpt.status).to.equal(1);
  });
  ...
  describe("Should deploy FeeDistributor and check initial state", function () {
    it("check initial state", async function () {
      ...
      await expect(
        feedistributor.connect(user0).setMerkleRoot(merkleRoot)
      ).to.be.revertedWith("Ownable: caller is not the owner");
      ...
      expect(await feedistributor.rewards()).to.equal(0);
    });
  });
  ...
  describe("Should deploy FeeDistributor and distribute/claim", function () {
    it("distribute/claim", async function () {
      ...
      await expect(
        feedistributor.connect(user0).distribute()
      ).to.be.revertedWith("Ownable: caller is not the owner");

      rcpt = await (await feedistributor.distribute()).wait();
      expect(rcpt.status).to.equal(1);
      ...
      expect(await litxtoken.balanceOf(feedistributor.address)).to.equal(100001);

      expect(await feedistributor.rewards()).to.equal(100001);
      expect(await feedistributor.isClaimed(merkleRoot, user0.address)).to.equal(false);
      ...
      rcpt = await (await feedistributor.connect(user0).claim(index1, claim1, proof1)).wait();
      expect(rcpt.status).to.equal(1);
