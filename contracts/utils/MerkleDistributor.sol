...
abstract contract MerkleDistributor {
    ...
    function isClaimed(bytes32 merkleRoot, uint256 index) public view returns (bool) {
    ...
    function _setClaimed(bytes32 merkleRoot, uint256 index) private {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        mapping(uint256 => uint256) storage _claimedBitMap = claimedBitMap[merkleRoot];
    ...
    function _claim(
        ...
        require(!isClaimed(merkleRoot, index), 'MerkleDistributor: already claimed');
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(MerkleProofUpgradeable.verify(merkleProof, merkleRoot, node), 'MerkleDistributor: bad proof');
